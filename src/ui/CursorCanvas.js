...
export default class CursorCanvas {
  constructor() {
    ...
    this.animateParams = this.animateParams.bind(this);
    this.syncDpiMetrics = this.syncDpiMetrics.bind(this);
  ...
  mouseMove(e) {
    ...
    if (!this.hasPointer) {
      this.targetPos[0] = this.cursor[0];
      this.targetPos[1] = this.cursor[1];
  ...
  update() {
    if (!this.ctx || !this.hasPointer) return;
  ...
  drawLink() {
    ...
    this.ctx.beginPath();
    this.ctx.arc(0, 0, this.params.aInnerCircleSize, 0, 2 * Math.PI);
    this.ctx.closePath();
    this.ctx.fill();
    this.ctx.setTransform(1, 0, 0, 1, 0, 0);
  }
  ...
  drawPlayer() {
    ...
    this.ctx.fillStyle = this.colors.playerBg;
    this.ctx.fill();
  ...
  aIn() {
    this.animateParams(this.params.sliderInTime, {
      circleSize: 0,
      aInnerCircleSize: this.params.linkDotSize
    });
  }
  ...
  animateParams(duration, vars) {
    const gsapLib = window.gsap;
    if (gsapLib?.to) {
      // GSAP 3 API: gsap.to(target, { ...vars, duration })
      gsapLib.to(this.params, { ...vars, duration });
      return;
    }

    Object.assign(this.params, vars);
  }

  syncDpiMetrics() {
    this.params.circleSize = 26 * this.dpi;
    this.params.circleWidth = 0.9 * this.dpi;
    this.params.sliderRad = 50 * this.dpi;
    this.params.arrowOppenes = 10 * this.dpi;
    this.params.linkDotSize = 4 * this.dpi;
    this.params.playerBgRad = 52 * this.dpi;
  }
